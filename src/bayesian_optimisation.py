...
if __name__ == "__main__":    
    z_values = np.random.normal([0,4], [1,1], size=(5,2))
    
    maximisation_quantity = np.random.normal([0], [1], size=5)
    
    print(z_values)
    print(maximisation_quantity)
    
    lower_bound = np.array([-5.0, -5.0])
    upper_bound = np.array([5.0, 5.0])
        
    canidates = new_candidate(z_values, maximisation_quantity, lower_bound, upper_bound)
