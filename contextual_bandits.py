...
from src.bandit import get_bandit, ClassificationBandit, ButtonsBandit, SimpleContextBandit
...
parser.add_argument("--strategy", default="UCB", type=str)          #"UCB" or "TS"
...
class BanditClassificationExperiment:
    ...
    def create_bandit(self):
        ...
        if isinstance(self.bandit, SimpleContextBandit):
            print(f"Risk arm: {self.bandit.risk_arm}")
    ...
    def D_feature_stds(self):
        if not hasattr(self, "_D_feature_stds"):
            self._D_feature_stds = np.array(self.D_rows[self.feature_columns], dtype=float).std()
        #else:
    ...
    def get_next_z(self, z_idx: int, context: Optional[pd.DataFrame] = None, action: Optional[str|int] = None):
        if action is None: 
        ...
        for _ in range(100):
            if self.config.is_contextual_bandit:
                new_value = self.rng.normal(np.array([float(x) for x in list(context.values())]),
                                            (self.config.perturbation_std * self.D_feature_stds).flatten(), len(self.feature_columns))
    ...
    def get_single_trial_action(self, trial: int, context: Optional[pd.Series] = None, random_action: bool = False):
        if random_action:
            ...
            if self.config.num_permutations > 1:
                raise NotImplementedError("TS strategy is not implemented for multiple permutations.")
            for seed in range(self.config.num_permutations):
            
                permutation_seed = self.num_api_calls if self.use_api_call_seed else seed
                for action in tqdm(self.action_space):
                    permutation_seed = self.num_api_calls if self.use_api_call_seed else 0
                    query_note = BanditClassificationUtils.parse_features_and_action_to_note(\
                        action=action, row=pd.DataFrame([context]).iloc[0], feature_columns=self.feature_columns)
                    reward_sample = None
                    while reward_sample not in self.label_keys:
                        txt, probs = chat(
                            self.prompter.get_general_prompt(D_df=self.D_note_label_df,
                                                            query_note=query_note,
                                                            permutation_seed=permutation_seed), 
                        ...
                        self.num_api_calls += 1
                        reward_sample = txt[0]
                        print("reward sample", reward_sample)
                    
                    Q_values[action] = reward_sample
            max_Q_value = max(Q_values.values())
            max_Q_actions = [action for action, value in Q_values.items() if value == max_Q_value]
            
            action_taken = np.random.choice(max_Q_actions)
    ...
    def single_trial(self, trial: int):
        ...
        regret = self.bandit.get_optimal_mean_reward() - self.bandit.get_mean_reward(action_taken)

        print(f"Action: {action_taken}; Reward: {reward}; Regret: {regret}")
        context_copy = context.copy()
        
        if self.config.is_contextual_bandit:
            context = pd.DataFrame({key: [value] for key, value in context.items()})
            context["action"] = action_taken
            context["label"] = reward
            context["regret"] = regret
            context["note"] = BanditClassificationUtils.parse_features_and_action_to_note(action=action_taken, row=context_copy, feature_columns=self.feature_columns)
        ...
        if not is_random_action and not self.config.strategy == "TS":
        ...
        if trial == 0:
            self.D_rows = pd.DataFrame(context)
        else:
            self.D_rows = pd.concat([self.D_rows, pd.DataFrame(context)], ignore_index=True)
