...
def main():
    ...
    # Configuration
    data_dir = Path("TabLLM/datasets")
    data_dir = data_dir / args.dataset
    temp_output = 'dataset-generation-' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    output_dir = Path("TabLLM/datasets_serialized") / temp_output
    if not args.debug and not args.extract_dict:
        os.mkdir(output_dir)
    ...
    if args.extract_dict:
        extract_names(dataset, dataset_name)
        return
...
def extract_names(dataset, name):
    print(os.getcwd())
    dataset = dataset["test"]
    data_dict = {col: dataset[col].unique().tolist() for col in dataset.columns}
    with open(os.getcwd() + f'/TabLLM/datasets_serialized/{name}/names.json', 'w') as fp:
